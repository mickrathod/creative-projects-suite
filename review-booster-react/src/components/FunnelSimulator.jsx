...
export function FunnelSimulator({ onClose }) {
  const { bizConfig } = useReview();
  const [viewMode, setViewMode] = useState('phone'); // 'phone' or 'standalone'
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <span style={{ fontWeight: 800, fontSize: '14px' }}>Customer Mobile Review Simulator</span>
        </div>
      ...
      {viewMode === 'phone' ? (
        <div className="phone-simulator-frame">
          ...
          <div className="phone-screen">
            <CustomerFunnel key={key} customConfig={bizConfig} />
          </div>
        </div>
      ) : (
